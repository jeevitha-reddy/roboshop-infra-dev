resource "aws_cloudfront_distribution" "devrobo" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.devrobo.domain_name
        zone_id = aws_cloudfront_distribution.devrobo.hosted_zone_id
      }
    }
  ]
}